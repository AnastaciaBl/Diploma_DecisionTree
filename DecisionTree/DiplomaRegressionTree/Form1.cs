...
namespace DiplomaRegressionTree
{
    public partial class Form1 : Form
    {
        ...
        private void buildTree()
        {
            var qu = new Queue<DecisionTreeNode>();
            qu.Enqueue(Tree.Head);
            treeView.Nodes.Add(createTextOfRule(Tree.Head));
            int counterofLayer = 0;
            while (qu.Count != 0)
            {
                DecisionTreeNode tempNode = qu.Dequeue();
                if (!tempNode.IsLeaf)
                {
                    treeView.Nodes[counterofLayer].Nodes.Add(">" + createTextOfRule(tempNode.RightChild));
                    treeView.Nodes[counterofLayer].Nodes.Add("<=" + createTextOfRule(tempNode.LeftChild));
                    qu.Enqueue(tempNode.LeftChild);
                    qu.Enqueue(tempNode.RightChild);
                    counterofLayer++;
                }
        ...
        private string createTextOfRule(DecisionTreeNode node)
        {
            ...
            if (node.Rule.IsQualitative)
            {
                ...
                str = node.Rule.Rules[0].ToString();
            return str;
        }
